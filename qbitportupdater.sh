...
if [ -z $GLUETUN_API_URL ]; then
    ...
    echo "GLUETUN_API_URL is ${GLUETUN_API_URL}"
fi
...
if [ -z "$QBITTORRENT_URL" ]; then
    ...
    echo "QBITTORRENT_URL is ${QBITTORRENT_URL}"
fi
...
if [ -z "$QBITTORRENT_USERNAME" ]; then
    ...
    echo "QBITTORRENT_USERNAME is ${QBITTORRENT_USERNAME}"
fi
...
if [ -z "$QBITTORRENT_PASSWORD" ]; then
    ...
    echo "QBITTORRENT_PASSWORD is ${QBITTORRENT_PASSWORD}"
fi
...
if [ "$login_response_code" -ne 200 ]; then
    ...
    if [ "$preferences_response_code" -ne 200 ]; then
        ...
        if [ "$current_listen_port" -eq "$gluetun_forwarded_port" ]; then
            echo "Ports are already the same, no change needed"
        else
            # Set new listen port
